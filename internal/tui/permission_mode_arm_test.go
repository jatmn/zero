...
// later innocent ctrl+g commits unsafe mode with nobody having decided
...
func armedModel(t *testing.T) model {
	...
	if !armed.unsafeArmed {
		t.Fatal("shift+tab from Ask did not raise the unsafe offer")
	}
	if armed.permissionMode != agent.PermissionModeAsk {
		t.Fatalf("mode changed to %s while merely offering unsafe", armed.permissionMode)
	}
	return armed
}
...
// The confirm key immediately after the offer commits unsafe. This is the
// feature working.
func TestConfirmKeyCommitsUnsafeRightAfterTheOffer(t *testing.T) {
	...
	if m.permissionMode != agent.PermissionModeUnsafe {
		t.Fatalf("mode = %s after confirming, want unsafe", m.permissionMode)
	}
	if m.unsafeArmed {
...
func TestAnyOtherKeyCancelsTheUnsafeOffer(t *testing.T) {
	for name, key := range map[string]tea.Key{
		...
		t.Run(name, func(t *testing.T) {
			...
			if m.unsafeArmed {
				t.Fatalf("%s left the unsafe offer live, so a later ctrl+g would commit it silently", name)
			}
			// And prove the consequence rather than trusting the flag.
			m = pressKey(t, m, tea.Key{Code: 'g', Mod: tea.ModCtrl})
			if m.permissionMode == agent.PermissionModeUnsafe {
				t.Fatalf("ctrl+g after %s entered unsafe mode with no live offer", name)
			}
		})
	}
}
...
func TestConfirmKeyAloneNeverEntersUnsafe(t *testing.T) {
	for _, start := range []agent.PermissionMode{agent.PermissionModeAuto, agent.PermissionModeAsk} {
		...
		for press := 0; press < 3; press++ {
			m = pressKey(t, m, tea.Key{Code: 'g', Mod: tea.ModCtrl})
			if m.permissionMode == agent.PermissionModeUnsafe {
				t.Fatalf("ctrl+g alone from %s reached unsafe on press %d", start, press+1)
			}
		}
	}
}
...
func TestHoldingShiftTabNeverCommitsUnsafe(t *testing.T) {
	...
	for press := 0; press < 10; press++ {
		...
		if m.permissionMode == agent.PermissionModeUnsafe {
			t.Fatalf("shift+tab alone reached unsafe on press %d", press+1)
		}
	}
}
...
func TestOfferLabelNamesTheConfirmKey(t *testing.T) {
	...
	if label == "unsafe" {
		t.Fatal("the offer renders as though unsafe is already active")
	}
	for _, want := range []string{"unsafe", "ctrl+g"} {
